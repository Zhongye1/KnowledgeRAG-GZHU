...
import {
  ...
  TextInput, Switch, Alert, ActivityIndicator, Linking,
} from 'react-native';
...
const MODELS = [
  { id: 'ollama', label: 'Ollama（本地）', icon: '🦙', desc: '本地运行，无需费用' },
...
export default function SettingsScreen() {
  ...
  const handleSyncObsidian = async () => {
    if (!obsidianPath.trim()) { Alert.alert('请填写 Vault 路径'); return; }
    ...
    try {
      ...
      Alert.alert('同步完成', `新增 ${s.added ?? 0}，更新 ${s.updated ?? 0}，跳过 ${s.skipped ?? 0}`);
    } catch (e: any) {
  ...
  const handleSaveFeishu = async () => {
    if (!feishuAppId || !feishuSecret) { Alert.alert('请填写 App ID 和 App Secret'); return; }
    setFeishuLoading(true);
    try {
      const params = new URLSearchParams({ app_id: feishuAppId, app_secret: feishuSecret });
    ...
    <SafeAreaView style={styles.container} edges={['bottom']}>
      ...
      <View style={styles.tabBar}>
        {tabs.map(t => (
          ...
          >
            <Ionicons name={t.icon as any} size={16} color={activeTab === t.id ? COLORS.primary : COLORS.textMuted} />
        ...
        {activeTab === 'model' && (
          <View style={styles.section}>
            ...
            {MODELS.map(m => (
              <TouchableOpacity
                key={m.id}
                style={[styles.modelCard, selectedModel === m.id && styles.modelCardSelected]}
              ...
              >
                ...
                {selectedModel === m.id && <Ionicons name="radio-button-on" size={20} color={COLORS.primary} />}
              </TouchableOpacity>
            ))}
          ...
          <>
            ...
            <View style={styles.section}>
              ...
              <TextInput
                ...
                placeholderTextColor={COLORS.textMuted}
              />
              <Text style={[styles.fieldLabel, { marginTop: 10 }]}>目标知识库 ID（可选）</Text>
              <TextInput
                style={styles.textInput}
                value={obsidianKbId}
              ...
              />
              <TouchableOpacity
                style={[styles.primaryBtn, obsidianSyncing && styles.primaryBtnDisabled, { marginTop: 14 }]}
                onPress={handleSyncObsidian} disabled={obsidianSyncing}
              >
                {obsidianSyncing
                  ? <ActivityIndicator size="small" color="white" />
                  : <Ionicons name="sync-outline" size={16} color="white" />}
                <Text style={styles.primaryBtnText}>{obsidianSyncing ? '同步中...' : '配置并同步'}</Text>
              </TouchableOpacity>
            </View>
            ...
            <View style={styles.section}>
              ...
              <TextInput
                ...
                placeholderTextColor={COLORS.textMuted}
              />
              <Text style={[styles.fieldLabel, { marginTop: 10 }]}>App Secret</Text>
              <TextInput
                style={styles.textInput}
                value={feishuSecret}
              ...
              />
              <TouchableOpacity
                style={[styles.primaryBtn, feishuLoading && styles.primaryBtnDisabled, { marginTop: 14 }]}
                onPress={handleSaveFeishu} disabled={feishuLoading}
              >
                {feishuLoading
                  ? <ActivityIndicator size="small" color="white" />
              ...
              <View style={styles.webhookBox}>
                ...
                <Text style={styles.webhookUrl} numberOfLines={2}>{webhookUrl}</Text>
                <TouchableOpacity
                  onPress={() => {
...
const styles = StyleSheet.create({
  ...
  modelCardSelected: { borderColor: COLORS.primary, backgroundColor: COLORS.primary + '08' },
  ...
  webhookBox: {
    marginTop: 14, backgroundColor: '#f8fafc', borderRadius: RADIUS.sm,
    padding: SPACING.sm, borderWidth: 1, borderColor: COLORS.border,
  },
  webhookLabel: { fontSize: 12, color: COLORS.textMuted, marginBottom: 4 },
  webhookUrl: { fontSize: 11, color: '#1e40af', fontFamily: 'monospace', marginBottom: 8 },

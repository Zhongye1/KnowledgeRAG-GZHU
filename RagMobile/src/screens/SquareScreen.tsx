...
const GRADIENTS = ['#667eea', '#f093fb', '#4facfe', '#43e97b', '#fa709a', '#a18cd1'];
function coverColor(id: number) { return GRADIENTS[id % GRADIENTS.length]; }
...
// ── 知识库卡片 ───────────────────────────────────────────────
function KbCard({ kb, onStar, starred }: { kb: SharedKb; onStar: (kb: SharedKb) => void; starred: boolean }) {
  return (
    <View style={styles.card}>
      {/* 封面色块 */}
      <View style={[styles.cardCover, { backgroundColor: kb.cover_color || coverColor(kb.id) }]}>
        <Text style={styles.cardCoverText}>{kb.kb_name[0]}</Text>
      </View>
      {/* 内容区 */}
      <View style={styles.cardBody}>
        <Text style={styles.cardTitle} numberOfLines={1}>{kb.kb_name}</Text>
        {kb.description ? (
          <Text style={styles.cardDesc} numberOfLines={2}>{kb.description}</Text>
        ) : null}
        {/* 标签 */}
        {kb.tags.length > 0 && (
        ...
        <View style={styles.cardFooter}>
          <View style={styles.authorRow}>
            <View style={styles.authorAvatar}>
              <Text style={styles.authorInitial}>{(kb.author_name || '?')[0]}</Text>
            </View>
            <Text style={styles.authorName} numberOfLines={1}>{kb.author_name}</Text>
          </View>
...
// ── 圈子卡片 ─────────────────────────────────────────────────
function CircleCard({ circle, onJoin }: { circle: Circle; onJoin: (c: Circle) => void }) {
  return (
    <View style={styles.circleCard}>
      ...
      <View style={styles.circleInfo}>
        ...
        <Text style={styles.circleMeta}>{circle.member_count} 成员 · {circle.kb_count} 知识库</Text>
      </View>
      <TouchableOpacity
        style={[styles.joinBtn, circle._joined && styles.joinBtnActive]}
        onPress={() => onJoin(circle)}
      >
        <Text style={[styles.joinBtnText, circle._joined && styles.joinBtnTextActive]}>
...
export default function SquareScreen() {
  ...
  // ── 获取知识库列表 ───────────────────────────────────────
  const fetchKbs = useCallback(async (reset: boolean, keyword: string, cat: string, sort: string, pg: number) => {
    try {
      const params = new URLSearchParams({
        category: cat, sort, keyword, page: String(pg), page_size: '10',
      });
  ...
  const loadKbs = useCallback(async (reset = true) => {
    if (reset) {
      setLoading(true);
      setPage(1);
    }
  ...
  const handleStar = useCallback(async (kb: SharedKb) => {
    try {
      const res = await api.post(`/api/square/kbs/${kb.id}/star`, { user_id: 'mobile_user' });
      setStarred(prev => {
        const next = new Set(prev);
        if (res.data.starred) next.add(kb.id); else next.delete(kb.id);
        return next;
      });
      setKbList(prev => prev.map(k =>
        k.id === kb.id
  ...
  const handleJoin = useCallback(async (circle: Circle) => {
    try {
      if (circle._joined) {
        await api.delete(`/api/square/circles/${circle.id}/join?user_id=mobile_user`);
        setCircles(prev => prev.map(c =>
          c.id === circle.id ? { ...c, _joined: false, member_count: Math.max(1, c.member_count - 1) } : c
        ));
      } else {
        await api.post(`/api/square/circles/${circle.id}/join`, { user_id: 'mobile_user' });
        setCircles(prev => prev.map(c =>
          c.id === circle.id ? { ...c, _joined: true, member_count: c.member_count + 1 } : c
        ));
      }
    } catch (e: any) {
  ...
  const handleSearchChange = useCallback((text: string) => {
    ...
    searchTimer.current = setTimeout(() => {
      setPage(1);
      setLoading(true);
      fetchKbs(true, text, activeCat, sortBy, 1).then(() => setLoading(false));
    }, 600);
  ...
  // ── 渲染 ─────────────────────────────────────────────────
  const renderItem = useCallback(({ item }: { item: SharedKb }) => (
    ...
    <View>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.catScroll} contentContainerStyle={styles.catContent}>
        {CATEGORIES.map(cat => (
          ...
          >
            <Text style={[styles.catTabText, activeCat === cat.id && styles.catTabTextActive]}>
              {cat.label}
            </Text>
          </TouchableOpacity>
        ))}
      ...
      <View style={styles.actionBar}>
        <TouchableOpacity style={styles.actionBtn} onPress={() => setShowCircles(true)}>
        ...
        <TouchableOpacity style={styles.actionBtn} onPress={() => setShowSort(true)}>
          ...
          <Text style={styles.actionBtnText}>{SORT_OPTIONS.find(s => s.value === sortBy)?.label}</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
  ...
  return (
    <View style={styles.container}>
      ...
      {/* 圈子弹窗 */}
      <Modal visible={showCircles} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setShowCircles(false)}>
        <View style={styles.modalContainer}>
          ...
          <FlatList
            data={circles}
            renderItem={({ item }) => <CircleCard circle={item} onJoin={handleJoin} />}
            keyExtractor={item => String(item.id)}
            ListEmptyComponent={
              <View style={styles.emptyState}>
...
const styles = StyleSheet.create({
  ...
  // 卡片
  card: {
    backgroundColor: COLORS.card, borderRadius: RADIUS.lg, marginHorizontal: SPACING.md,
    marginTop: SPACING.sm, overflow: 'hidden', ...SHADOW.md,
  },
  cardCover: {
    height: 110, alignItems: 'center', justifyContent: 'center',
  },
  cardCoverText: { fontSize: 40, fontWeight: '800', color: 'rgba(255,255,255,0.85)' },
  cardBody: { padding: 12 },
  ...
  tag: {
    ...
    borderRadius: 4, paddingHorizontal: 6, paddingVertical: 2, marginRight: 4,
  },
  ...
  noMoreText: { textAlign: 'center', color: COLORS.textMuted, fontSize: 12, padding: 16 },
  ...
  sortTitle: { fontSize: 16, fontWeight: '700', color: COLORS.text, marginBottom: SPACING.md },
  sortOption: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',

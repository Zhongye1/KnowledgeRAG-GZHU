...
class PDFExportTool(AgentTool):
    ...
    def run(self, content: str = "", output_path: str = "output.pdf",
            title: str = "报告") -> str:
        try:
            import markdown
...
class TranslateTool(AgentTool):
    ...
    def run(self, text: str = "", target_lang: str = "en") -> str:
        try:
            import urllib.request, urllib.parse, hashlib, random
...
def register_all_tools():
    tools = [
        DataAnalysisTool(), PythonCodeTool(), ChartGeneratorTool(),
        EmailTool(), FileReadTool(), FileWriteTool(), PDFExportTool(),
        CalendarTool(), TranslateTool(), SummarizeTool(), KeywordExtractTool(),
    ]
    for tool in tools:

...
### 15.1 Redis Stream 消息队列

**解释：** 用 Redis Stream 替代内存队列，实现任务持久化、重试机制和死信队列，服务重启后任务不丢失。

**原本应用方向：** 作为向量化任务的持久化队列层，解耦上传接口与向量化处理，支持跨服务/进程的任务调度。

**跳过原因：** 项目为本地/校园部署场景，引入 Redis 增加一个强依赖服务，Docker 复杂度翻倍。现有 `asyncio.Queue` + `asyncio.to_thread` 方案已能解决核心问题（上传接口不阻塞、向量化并发受控），且向量化结果本身持久化在磁盘，服务重启后仅丢失进度状态，可接受。
...
**跳过原因：** 子进程间通信（IPC）开销较大，进程创建/销毁本身也有开销。对 Ollama 这个天然慢操作（几秒到几十秒/文件）来说，进程开销占比可忽略但收益有限。`asyncio.to_thread` 的线程池模式已足够隔离阻塞操作，Semaphore 限制并发数从源头控制内存峰值。
...
### 15.4 全链路超时统一（Nginx 600s 配置）

**解释：** 将前端 Axios、Nginx 反向代理、FastAPI Uvicorn 的所有超时时间统一设置为 600 秒，彻底消除中间环节断连。

**原本应用方向：** 防止长时间向量化操作（如 100 个文档批量处理）触发中间层超时导致 Network Error。

**跳过原因：** 向量化接口已改为异步（立即返回 task_id，后台处理），HTTP 请求本身毫秒级返回，不再需要长超时。统一 600s 反而会掩盖真实的连接问题，不利于排查故障。轮询 `/api/vectorize/status/{task_id}` 的请求本身响应极快，无超时风险。

...
本项目使用 **Ollama** 在本地运行大语言模型，运行前请先完成以下准备：

**1. 安装 Ollama**

前往 [https://ollama.com](https://ollama.com) 下载并安装对应系统的版本。

**2. 拉取所需模型**

```bash
# 主问答模型（推荐：DeepSeek-R1 7B 蒸馏版）
# 显存要求：约 4.7GB VRAM（8GB 显卡可运行）
# 内存要求：8GB 以上
...
> 如果你的硬件更强，也可以换用更大的模型（如 `deepseek-r1:14b`），只需修改 `.env` 中的 `MODEL` 变量即可。

<template>
  <div class="graph-container flex flex-col gap-3">
    ...
    <div class="toolbar flex flex-wrap items-center gap-2">
      ...
      <div class="flex items-center gap-1">
        ...
        <button v-if="isSearchMode" @click="clearSearch"
          class="text-gray-500 hover:text-red-500 text-sm px-1" title="清除搜索">✕</button>
      </div>
      ...
      <div v-if="showStats && graphStats"
        ...
        <div class="flex justify-between items-center mb-2">
          ...
          <button @click="showStats = false" class="text-gray-400 hover:text-gray-600">✕</button>
        </div>
        <div class="space-y-1 text-gray-600">
          <div>节点总数：<span class="font-medium text-blue-600">{{ graphStats.node_count }}</span></div>
          <div>边总数：<span class="font-medium text-green-600">{{ graphStats.edge_count }}</span></div>
          <div>孤立节点：<span class="font-medium text-orange-500">{{ graphStats.isolated_node_count }}</span></div>
          <div v-if="graphStats.type_distribution" class="mt-2">
            <div class="text-xs text-gray-500 mb-1">节点类型分布：</div>
            <div v-for="(count, type) in graphStats.type_distribution" :key="type"
              class="flex justify-between text-xs">
              <span>{{ type }}</span><span class="font-medium">{{ count }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- 错误提示 -->
    <div v-if="errorMessage" class="text-red-500 text-sm p-2 bg-red-50 border border-red-200 rounded">
      {{ errorMessage }}
    </div>
      ...
      <transition name="slide">
        <div v-if="selectedNode"
          ...
          <div class="flex justify-between items-center mb-2">
            ...
            <button @click="selectedNode = null" class="text-gray-400 hover:text-gray-600">✕</button>
          </div>
          <div class="space-y-1 text-gray-600">
            <div><span class="text-gray-400">ID：</span>{{ selectedNode.id }}</div>
            <div><span class="text-gray-400">名称：</span>
            ...
            <div class="mt-2 pt-2 border-t text-xs">
              ...
              <div v-for="rel in getNodeRelations(selectedNode.id)" :key="rel.key"
...
// ── 类型定义
interface GraphNode { id: string; label?: string; type?: string; x?: number; y?: number; size?: number; color?: string; }
interface GraphEdge { source: string; target: string; label?: string; }
interface GraphData { nodes: GraphNode[]; edges: GraphEdge[]; }
interface GraphStats { node_count: number; edge_count: number; isolated_node_count: number; type_distribution?: Record<string, number>; }
...
const NODE_COLORS: Record<string, string> = {
  '人物': '#FF6B6B', '地点': '#4ECDC4', '组织': '#45B7D1',
  '概念': '#FFD166', '事件': '#06D6A0', '默认': '#90D8FF',
};
...
const getNodeRelations = (nodeId: string) => {
  const rels: Array<{ key: string; direction: string; source: string; target: string; label: string }> = [];
  for (const e of currentEdges) {
    if (e.source === nodeId) {
      rels.push({ key: `out-${e.source}-${e.target}`, direction: 'out', source: e.source, target: e.target, label: e.label || '' });
    } else if (e.target === nodeId) {
      rels.push({ key: `in-${e.source}-${e.target}`, direction: 'in', source: e.source, target: e.target, label: e.label || '' });
    }
...
const fetchGraphData = async () => {
  const kbId = props.kbId || String(route.params.id || '');
  if (!kbId) { errorMessage.value = '未提供知识库 ID'; return; }
...
const searchNodes = async () => {
  ...
  try {
    ...
    if (data.message) { errorMessage.value = data.message; return; }

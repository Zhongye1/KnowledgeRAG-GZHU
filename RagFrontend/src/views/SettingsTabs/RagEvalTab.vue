<template>
  <div class="tab-content">
    <div class="section-header">
      <h2>模型评测面板</h2>
      <p class="section-desc">基于内置 20 条中文测试题，从准确率 / 响应速度 / 溯源准确率三维度评测，ECharts 可视化对比</p>
    </div>
    ...
    <div class="eval-history-card">
      ...
      <div v-if="!historyList.length" class="eval-empty">暂无评测记录，点击「开始评测」触发首次评测</div>
      <table v-else class="eval-table">
        <thead>
          <tr>
            <th>ID</th><th>模型</th><th>时间</th><th>准确率</th>
            <th>avg延迟</th><th>溯源率</th><th>综合分</th><th>状态</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="r in historyList" :key="r.id" @click="viewDetail(r.id)" class="eval-tr">
            ...
            <td><span :class="scoreClass(r.accuracy)">{{ pct(r.accuracy) }}</span></td>
            <td>{{ r.avg_latency?.toFixed(0) }}ms</td>
            <td>{{ pct(r.source_acc) }}</td>
            <td><b :class="scoreClass(r.overall)">{{ pct(r.overall) }}</b></td>
            <td><span :class="['eval-status', r.status === 'done' ? 'eval-status--ok' : 'eval-status--run']">
    ...
    <div class="eval-qs-card">
      ...
      <div class="eval-qs-list">
        ...
        <button v-if="allQuestions.length > 5" class="eval-btn-more" @click="showAllQs = !showAllQs">
...
const overviewCards = computed(() => {
  ...
  return [
    { key: 'acc',    icon: '🎯', label: '准确率',    value: pct(r.accuracy),   color: scoreColor(r.accuracy) },
    { key: 'lat',    icon: '⚡', label: '平均响应',   value: `${r.avg_latency?.toFixed(0)}ms`, color: r.avg_latency < 2000 ? '#22c55e' : '#f59e0b' },
    { key: 'src',    icon: '📎', label: '溯源准确率', value: pct(r.source_acc), color: scoreColor(r.source_acc) },
    { key: 'overall',icon: '🏆', label: '综合评分',   value: pct(r.overall),   color: scoreColor(r.overall) },
...
const scoreClass = (v: number) => v >= 0.7 ? 'score--good' : v >= 0.4 ? 'score--mid' : 'score--bad'
...
function renderLatencyHist(echarts: any) {
  ...
  chart.setOption({
    ...
    series: [{
      type: 'bar',
      data: d.counts,
      itemStyle: { color: (p: any) => p.dataIndex <= 1 ? '#22c55e' : p.dataIndex <= 3 ? '#f59e0b' : '#ef4444' }
    }]
  })
}
...
function ensureECharts(): Promise<void> {
  return new Promise(resolve => {
    if ((window as any).echarts) { resolve(); return }
...
.eval-model-input { display: flex; flex-direction: column; gap: 4px; flex: 1; min-width: 220px; }
.eval-model-input label { font-size: 12px; color: #6b7280; }
.eval-input {
  padding: 8px 12px;
...
.eval-input:focus { border-color: #6366f1; }
.eval-btn-run {
  padding: 8px 22px;
  background: #6366f1;
  color: #fff;
...
.eval-btn-run:disabled { opacity: .6; cursor: not-allowed; }
.eval-btn-run:hover:not(:disabled) { background: #4f46e5; }
.eval-btn-refresh {
  padding: 8px 14px;
  background: #f9fafb;
  color: #374151;
...
.eval-btn-refresh:hover:not(:disabled) { background: #f3f4f6; }
...
.eval-ov-icon { font-size: 20px; }
.eval-ov-value { font-size: 20px; font-weight: 700; }
.eval-ov-label { font-size: 11px; color: #6b7280; }
...
@media (max-width: 900px) { .eval-charts-row { grid-template-columns: 1fr; } }
.eval-chart-box {
  background: var(--td-bg-color-container, #fff);
  border: 1px solid #e5e7eb;
  border-radius: 10px;
...
.eval-chart-title { font-size: 13px; font-weight: 600; margin-bottom: 8px; color: var(--td-text-color-primary, #111); }

.eval-history-card, .eval-qs-card {
  background: var(--td-bg-color-container, #fff);
  border: 1px solid #e5e7eb;
  border-radius: 10px;
  padding: 16px;
...
.eval-history-title { font-size: 14px; font-weight: 600; margin-bottom: 12px; }
.eval-empty { font-size: 13px; color: #9ca3af; text-align: center; padding: 20px; }
.eval-table { width: 100%; border-collapse: collapse; font-size: 12px; }
.eval-table th { text-align: left; padding: 6px 10px; background: #f9fafb; border-bottom: 1px solid #e5e7eb; }
.eval-tr td { padding: 7px 10px; border-bottom: 1px solid #f3f4f6; cursor: pointer; }
.eval-tr:hover { background: #f9fafb; }
.eval-status { display: inline-block; padding: 2px 8px; border-radius: 10px; font-size: 11px; }
.eval-status--ok { background: #dcfce7; color: #166534; }
.eval-status--run { background: #dbeafe; color: #1e40af; }
.score--good { color: #22c55e; font-weight: 600; }
.score--mid  { color: #f59e0b; font-weight: 600; }
.score--bad  { color: #ef4444; font-weight: 600; }

.eval-qs-cats { display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 12px; }
.eval-cat-chip {
  padding: 3px 10px;
  background: #eff6ff;
  color: #1d4ed8;
...
.eval-qs-list { display: flex; flex-direction: column; gap: 6px; }
.eval-q-row { display: flex; align-items: center; gap: 8px; font-size: 13px; }
.eval-q-cat {
  min-width: 80px;
  font-size: 11px;
  color: #6b7280;
